...
class akkumatik_display:
    ...
    def filesplit(self, fh):
        ...
        #TODO won't work on windoof probably (and it's not good anyway)
        shutil.copy(exe_dir + "/serial-akkumatik.dat", exe_dir + "/.tmp")

        #pos = fh.tell()

        #tmpf = os.tmpfile()

        fhI = self.open_file(exe_dir + '/.tmp', "r")
