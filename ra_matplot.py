...
def matplot():
    def hms_formatter(self, value, loc):
        h = value // 3600        
        m = (value - h * 3600) // 60
        s = value % 60              
        return "%02d:%02d:%02d" % (h,m,s)

    def to_sec(self, tstring):
    ...
    for fname in dirList:
        if fname[0:4] == "Akku" and fname[4:6] == str(self.gewaehlter_ausgang) + "-" and fname [8:12] == ".dat":
            ...
            # file to array
            fh  = self.open_file(self.tmp_dir + "/" + fname, "r")
            total_a = []
        ...
        #break
            ...
            xaxis = sp1.get_xaxis()
            xaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(self.hms_formatter))
            sp1.plot(t, y)

            sp2 = fig.add_subplot(212)
            xaxis2 = sp2.get_xaxis()
            xaxis2.set_major_formatter(matplotlib.ticker.FuncFormatter(self.hms_formatter))
            sp2.plot(t, y2)
